...
@LYCIA.on_message(filters.command("mcube"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("MCubeMedia AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("MCubeMedia Is Best...")
    try:
        L = await fetch(
