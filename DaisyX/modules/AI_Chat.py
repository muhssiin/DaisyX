...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"MCubeMedia AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("MCubeMedia AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"MCubeMedia AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("mcube", "Aco")
        test = test.replace("MCube", "Aco")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "mcube")
        response = response.replace("aco", "MCube")
        ...
        response = response.replace("Aco", "MCube")
        response = response.replace("aco", "MCube")
        response = response.replace("Luna", "MCube")
        response = response.replace("luna", "MCube")
        pro = response
        if not "en" in lan and not lan == "":
            try:
...
__help__ = """
<b> Chatbot </b>
MCUBE AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
...
<b> Assistant </b>
 - /ask [question]: Ask question from MCubeMedia
